...
- [x] Page **Détail devis** : édition des lignes (modifier/ajouter/supprimer, sauvegarde atomique via RPC `update_devis_lines`), changement de statut (`brouillon` → `envoyé` → `accepté`/`refusé`, retour `envoyé` → `brouillon`) — nécessite `supabase/migrations/0007_update_devis_lines_rpc.sql` — **testé manuellement par l'utilisateur** : navigation depuis le Dashboard, édition/ajout de lignes sauvegardé, passage envoyé → accepté avec mise à jour du badge Dashboard via Realtime
- [x] Page **Clients** : liste + formulaire inline (création/édition, pas de suppression — `on delete cascade` vers devis/chantiers/factures trop risqué pour ce chantier), aucune nouvelle migration/policy nécessaire (`clients_all` déjà en place) — **testé manuellement par l'utilisateur** : création, édition depuis la liste, nouveau client bien sélectionnable dans le menu déroulant de Nouveau devis
- [x] Page **Suivi relances** : lecture de `relances_log` (`cible_id` n'étant pas une vraie FK, résolution du numéro de devis via une requête séparée côté client), aucune nouvelle migration/policy nécessaire (`relances_log_all` déjà en place) — **testé manuellement par l'utilisateur** : état vide correct, ligne de test insérée en base affichée avec numéro de devis résolu et badge de statut corrects
